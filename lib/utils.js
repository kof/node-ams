 ...
 * @param {string} hostPath path to the file, which is trying to resolve.
 ...
 */
exports.resolvePath = function(path, hostPath, root, lookup) {
    var resolvedPath, 
        firstChar;
    ...
    // ./path/to/file.css, ../path/to/file.css    
        ...
        resolvedPath = n.path.join(n.path.dirname(hostPath), path);
    // use lookup    
    } else if (Array.isArray(lookup)) {
        lookup.some(function(lookupPath) {
            var curesolvedPath = n.path.join(lookupPath, path);
            try {
                if (n.fs.statSync(curesolvedPath).isFile()) {
                    resolvedPath = curesolvedPath;
                    return true;
                }
            } catch(e) {}
        });            
    ...
    if (!resolvedPath || !n.fs.statSync(resolvedPath).isFile()) {
        throw new Error('resolved path is not a file: ' + resolvedPath);    
    }
